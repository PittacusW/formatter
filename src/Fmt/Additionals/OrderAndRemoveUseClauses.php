...
EOT;
	...
	protected function sortUseClauses($source, $splitComma, $removeUnused, $stripBlankLines, $blanklineAfterUseBlock) {
		...
		while (list($index, $token) = each($tokens)) {
			...
			if ((T_TRAIT === $id || T_CLASS === $id) && !$touchedDoubleColon) {
				$newTokens[] = $token;
			...
			if (!$stripBlankLines && (T_WHITESPACE === $id || (T_COMMENT === $id && '/' == $text[2])) && substr_count($text, $this->newLine) >= 2) {
				++$groupCount;
				$useStack[$groupCount] = [];
				$newTokens[] = $token;
				continue;
		...
		if ($removeUnused) {
			$unusedImport = array_keys(array_filter($aliasCount, function ($v) {
				return 0 === $v;
			}));
		}
	...
	private function sortWithinNamespaces($source) {
		...
		while (list($index, $token) = each($tokens)) {
			...
			case T_NAMESPACE:
				...
				$return .= str_replace(self::OPENER_PLACEHOLDER, '', $this->sortUseClauses(self::OPENER_PLACEHOLDER . $namespaceBlock, self::SPLIT_COMMA, self::REMOVE_UNUSED, self::STRIP_BLANK_LINES, self::BLANK_LINE_AFTER_USE_BLOCK));
				break;
			default:
				$return .= $text;
			}
