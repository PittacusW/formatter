...
class OrganizeClass extends AdditionalPass {
	...
	public function candidate($source, $foundTokens) {
		...
		if (isset($foundTokens[T_CLASS]) || isset($foundTokens[T_TRAIT]) || isset($foundTokens[T_INTERFACE])) {
			return true;
		}
		return false;
	}
	public function format($source) {
		...
		while (list($index, $token) = each($this->tkns)) {
			...
			case T_TRAIT:
				...
				$return .= str_replace(self::OPENER_PLACEHOLDER, '', static::orderMethods(self::OPENER_PLACEHOLDER . $classBlock));
		...
		while (list($index, $token) = each($tokens)) {
			...
			case T_COMMENT:
				...
				if (strpos($text, "\x2") === false) {
					...
					if ($this->rightTokenSubsetIsAtIdx($tokens, $this->ptr, [T_ABSTRACT, T_FUNCTION, T_PRIVATE, T_PROTECTED, T_PUBLIC, T_STATIC], $this->ignoreFutileTokens)) {
			...
			case T_FINAL:
				...
				$attributeName = '';
				$visibilityLevel = 0;
				$searchFor = [T_ABSTRACT, T_FUNCTION, T_PRIVATE, T_PROTECTED, T_PUBLIC, T_STATIC, T_STRING, T_VARIABLE, T_FINAL];
				prev($tokens);
				do {
