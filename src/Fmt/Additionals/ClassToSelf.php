...
class ClassToSelf extends AdditionalPass {
	...
	public function candidate($source, $foundTokens) {
		...
		if (isset($foundTokens[T_CLASS]) || isset($foundTokens[T_INTERFACE]) || isset($foundTokens[T_TRAIT])) {
			return true;
		}
		return false;
	}
...
EOT;
	...
	private function convertToPlaceholder($name, $start, $end) {
		for ($i = $start; $i < $end; ++$i) {
			...
			if (!(T_STRING == $id && strtolower($text) == strtolower($name)) || $this->leftTokenSubsetIsAtIdx($this->tkns, $i, T_NS_SEPARATOR) || $this->rightTokenSubsetIsAtIdx($this->tkns, $i, T_NS_SEPARATOR)) {
				continue;
			}

			if ($this->leftTokenSubsetIsAtIdx($this->tkns, $i, [T_INSTANCEOF, T_NEW]) || $this->rightTokenSubsetIsAtIdx($this->tkns, $i, T_DOUBLE_COLON)) {
