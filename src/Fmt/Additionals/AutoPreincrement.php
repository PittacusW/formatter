...
class AutoPreincrement extends AdditionalPass {
	...
	public function format($source) {
		...
		for ($this->ptr = sizeof($this->tkns) - 1; $this->ptr >= 0; --$this->ptr) {
			...
			if (!$this->leftUsefulTokenIs([ST_BRACKET_CLOSE, ST_CURLY_CLOSE, T_STRING, T_VARIABLE]) || !$this->rightUsefulTokenIs([ST_SEMI_COLON, ST_PARENTHESES_CLOSE])) {
				continue;
			}
			$this->findVariableLeftEdge();

			if ($this->leftUsefulTokenIs([ST_SEMI_COLON, ST_CURLY_OPEN, ST_CURLY_CLOSE, T_OPEN_TAG])) {
