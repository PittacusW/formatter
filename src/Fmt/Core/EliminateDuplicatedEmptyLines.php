...
final class EliminateDuplicatedEmptyLines extends FormatterPass {
	...
	public function format($source) {
		...
		$count = 0;
		do {
			$ret = str_replace(self::EMPTY_LINE . $this->newLine . self::EMPTY_LINE . $this->newLine . self::EMPTY_LINE . $this->newLine, self::EMPTY_LINE . $this->newLine . self::EMPTY_LINE . $this->newLine, $ret, $count);
		} while ($count > 0);
