	...
	private function scanUntilEqual($index) {
		for ($index; 0 <= $index; --$index) {
			...
			case T_WHITESPACE:
				...
				if ($this->hasLn($text) && !($this->rightUsefulTokenIs([ST_SEMI_COLON]) || $this->leftUsefulTokenIs([ST_BRACKET_OPEN, ST_COLON, ST_CURLY_CLOSE, ST_CURLY_OPEN, ST_PARENTHESES_OPEN, ST_SEMI_COLON, T_END_HEREDOC, T_OBJECT_OPERATOR, T_OPEN_TAG]) || $this->leftTokenIs([T_COMMENT, T_DOC_COMMENT]))) {
