	...
	public function format($source) {
		...
		while (list($index, $token) = each($this->tkns)) {
			...
			case '-':
				...
				if ($this->leftMemoUsefulTokenIs([T_INC, T_DEC, T_LNUMBER, T_DNUMBER, T_VARIABLE, ST_PARENTHESES_CLOSE, T_STRING, T_ARRAY, T_ARRAY_CAST, T_BOOL_CAST, T_DOUBLE_CAST, T_INT_CAST, T_OBJECT_CAST, T_STRING_CAST, T_UNSET_CAST, ST_BRACKET_CLOSE]) && $this->rightUsefulTokenIs([T_INC, T_DEC, T_LNUMBER, T_DNUMBER, T_VARIABLE, ST_PARENTHESES_OPEN, ST_PARENTHESES_CLOSE, T_STRING, T_ARRAY, T_ARRAY_CAST, T_BOOL_CAST, T_DOUBLE_CAST, T_INT_CAST, T_OBJECT_CAST, T_STRING_CAST, T_UNSET_CAST, ST_BRACKET_CLOSE, ST_BRACKET_OPEN])) {
					$this->appendCode(' ' . $text . ' ');
					break;
				}
				$this->appendCode($text);
			...
			case ST_CONCAT:
				...
				if (T_WHITESPACE === $prevId && T_WHITESPACE !== $nextId) {
					$this->appendCode($text . $this->getSpace(!$this->rightTokenIs(ST_COLON)));
					break;
				} elseif (T_WHITESPACE !== $prevId && T_WHITESPACE === $nextId) {
					$this->appendCode(' ' . $text);
					break;
				} elseif (T_WHITESPACE !== $prevId && T_WHITESPACE !== $nextId) {
					$this->appendCode(' ' . $text . $this->getSpace(!$this->rightTokenIs(ST_COLON)));
			...
			case ST_COLON:
				...
				if ($inTernaryOperator > 0 && T_WHITESPACE === $prevId && T_WHITESPACE !== $nextId) {
					$this->appendCode($text . ' ');
					--$inTernaryOperator;
					break;
				} elseif ($inTernaryOperator > 0 && T_WHITESPACE !== $prevId && T_WHITESPACE === $nextId) {
					...
					$this->appendCode($text . ' ');
					break;
				} elseif ((T_WHITESPACE != $nextId || (T_WHITESPACE == $nextId && !$this->hasLn($nextText))) && $this->rightUsefulTokenIs(T_CLOSE_TAG)) {
					$this->appendCode($text . ' ');
					break;
			...
			case ST_CURLY_OPEN:
				...
				if (!$touchedUse && $this->leftMemoUsefulTokenIs([T_VARIABLE, T_STRING]) && $this->rightUsefulTokenIs([T_VARIABLE, T_STRING])) {
					...
					$this->rtrimAndAppendCode(' ' . $text . $this->getSpace($this->rightTokenIs(T_COMMENT)));
					break;
			...
			case T_CONTINUE:
				...
				break;
			case T_CLASS:
				$this->appendCode($text . $this->getSpace(!($this->leftMemoUsefulTokenIs(T_NEW) && $this->rightTokenIs([ST_PARENTHESES_OPEN, T_EXTENDS, T_IMPLEMENTS])) && !$this->leftMemoTokenIs(T_DOUBLE_COLON) && !$this->rightTokenIs(ST_SEMI_COLON) || $this->rightTokenIs(T_DOUBLE_ARROW)));
				break;
			case T_EXTENDS:
			case T_IMPLEMENTS:
			case T_INSTANCEOF:
