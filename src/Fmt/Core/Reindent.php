...
final class Reindent extends FormatterPass {
	public function candidate($source, $foundTokens) {
		...
		if (isset($foundTokens[ST_CURLY_OPEN]) || isset($foundTokens[ST_PARENTHESES_OPEN]) || isset($foundTokens[ST_BRACKET_OPEN])) {
			return true;
		}
		return false;
	}
	public function format($source) {
		...
		while (list($index, $token) = each($this->tkns)) {
			...
			if ((T_WHITESPACE === $id || (T_COMMENT === $id && '//' == substr($text, 0, 2))) && $this->hasLn($text)) {
			...
			case ST_BRACKET_OPEN:
				$indentToken = ['id' => $id, 'implicit' => true];
				$this->appendCode($text);
			...
			case T_WHITESPACE:
				...
				if ($this->hasLn($text) && $this->rightTokenIs([T_COMMENT, T_DOC_COMMENT]) && $this->rightUsefulTokenIs([T_CASE, T_DEFAULT])) {
					$this->setIndent(-1);
